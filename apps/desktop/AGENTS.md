...
- **Transfer data plane lives in `main/transfer/`**:
  ...
  - `main/transfer/discovery.ts` — `DiscoveryTracker`, presence over the well-known `DISCOVERY_TOPIC` from `@rox/core`. The session joins the topic at startup (`joinDiscovery`), tracks every swarm connection, and pings/prunes on a 5 s interval (`PING_INTERVAL_MS`/`PEER_TIMEOUT_MS`); identity comes from the peer's `pair-hello`, liveness from `pair-ping`/`pair-pong`. The tracker only observes — pairing/transfer state stays in the session's `setActivePeer` path. Renderer surface: `peers:discover` (join + list, idempotent), `peers:list`, and `{ type: "peers", peers }` events on `transfer:event`; `useTransfer().onPeers` wraps both
  - **Direct send**: `transfer:send` takes an optional `deviceId` (a discovered peer id). The session seeds immediately, holds the offer in `queuedOffer`, and `flushQueuedOffer` delivers `hyper-offer` down the peer connection the moment pairing confirms (both accepts, trust-list auto-accept included) — flushed from the hello/accept handlers and `acceptLocal`. Unconfirmed pairings still block offers (existing safety rule); the receiver auto-receives once confirmed
  - **Optical beam (screen → camera)**: `main/transfer/beam.ts` registers `beam:prepare` — reads the file in Node, fountain-codes it via `@rox/core`'s optical codec, and returns the looping list of base64 QR frame payloads (header first, then deterministic symbols; TEXT_ENCODING, same wire format the native receiver scans). The renderer side is `features/transfer/beam-sender.tsx`: file pick → fullscreen black overlay cycling frames at 250 ms (matches native cadence), ESC/Enter/click exits, "not encrypted" notice shown on the picker. Entered via "Beam via screen (no network)" on the send wizard's device step. No camera/receive on desktop — that's a separate future decision
...
- **Env**: `main/env.ts` (first import in `main.ts`) loads `.env` via dotenv and validates through `@rox/env/desktop`; reads go through `env` from that module, never `process.env` (exception: `NODE_ENV` for the toolchain-set prod flag). `apps/desktop/.env` is gitignored; `.env.example` documents the vars (currently `ROX_USER_DATA_SUFFIX` for multi-instance userData isolation)
...
- New transfer/pairing UI goes in `renderer/features/<name>/`, one folder per feature; it calls only `window.ipc.transfer.*` (or `window.ipc.getPathForFile` for dropped files) — never the adapter directly. The optical beam is the exception: it uses `window.ipc.beam.prepare` (same rule, different namespace) Transfer UI is a **guided wizard**: `features/transfer/wizard.tsx` (shared shell/steps), `send-wizard.tsx` (nearby-device picker → dropzone → direct-send panel; picking a device sends straight to it and the offer is queued in the session until pairing confirms — no QR step. "Skip — share an offer instead" falls back to the QR offer flow. Both direct-send and QR flows end in a completion screen once the receiver's `hyper-release` confirms the transfer: a "File sent" panel with a "Send another file" button that resets the wizard), `receive-wizard.tsx` (scan/paste → done), with the mode chooser on `renderer/pages/home.tsx`. While idle on home, an incoming pairing from a discovered device surfaces as an accept/reject banner with the safety code (`IncomingPairingBanner` in `pages/home.tsx`) — that banner IS the receiver-side prompt for direct sends. Dropped files resolve to paths via the preload's `getPathForFile` (Electron `webUtils`)

...
export function parseControlMessage(raw: string): ControlMessage {
  ...
  if (msg.type !== "start" && msg.type !== "cancel") {
    throw new Error(`Unknown control message type: ${String((msg as { type?: string }).type)}`);
  }
  return msg;
}
...
export function encodeChunk(chunk: Chunk): ArrayBuffer {
  ...
  const frame = new ArrayBuffer(12 + fileIdBytes.byteLength + chunk.payload.byteLength);
